  ...
  if (size(n,1) == 2 && size(n,2) == 2)
    ...
    for i = 1:(size(n, 1) / 2)
      for j = 1:(size(n, 2) / 2)
        ...
        ans(i + size(n,1)/2, j + size(n,1)/2) = value_first(i,j) - value_second(i,j)*W(log2(size(n,1)),j - 1) - value_thirst(i,j)*W(log2(size(n,1)),i - 1) + value_last(i,j)*W(log2(size(n,1)),i+j - 2);
      endfor
    endfor
  end
endfunction
